...
![](imagenes/monitoring/ciuo_grupos.png){.fragment .fade-in-then-out fig-align="center" width="80%"}

![](imagenes/monitoring/caenes_secciones.png){.fragment .fade-in-then-out fig-align="center" width="100%"}
...
- Solución: **monitoreo periódico y re-entrenamiento de los modelos**.
  ...
  - Hacemos uso de estas glosas para implementar el monitoreo: 
    - **comparando el nuevo dataset mensual (producción) versus el usado para entrenar los modelos (referencia)**, evaluando la existencia de **data drift**. 
:::
...
- **Cómo calculamos una única métrica a partir de las múltiples métricas aplicadas?**
  - Usamos sumas ponderadas para combinar las métricas obtenidas a distintos niveles (registro, clase, dataset) y luego para obtener una métrica final de revisión (**revision_score**).
:::
...
::: {.incremental .medium-par}
- **¿Cómo validamos la métrica calculada (revision_score)?**
  - Seleccionamos un subconjunto de registros para cada periodo de interés (ej: coyunturas) para ser codificado a uno y dos dígitos por analistas expertos.
    - 100 registros para cada tipo de modelo: CIUO y CAENES.
    - Cada selección contiene 50 registros observados en los valores menores y mayores del **revision_score** en el periodo, ordenados aleatoriamente.
    - Distribución de clases (predichas) es la misma que en el dataset de producción del periodo.
  - Dos analistas clasifican el mismo subconjunto de registros seleccionados.
    - Permite contar con dos clasificaciones independientes y contrastarlas.
  - Los analistas además responden un par de preguntas que nos permitirán evaluar la **complejidad** y **novedad** de los registros, a partir de su conocimiento experto.
...
::: {.incremental .medium-par}
- **¿Cómo validamos la métrica calculada (revision_score)?**
  - Analizaremos cómo se correlacionan nuestra métrica a 1 dígito con:
    - **La estimación de complejidad (Q1) y novedad (Q2) por parte de analistas.**
...
::: {.medium-par}
**Q1: ... ¿qué tan difícil fue clasificar este registro?
** <br><br>
...
::: {.incremental .medium-par}
- **Correlación entre nuestra métrica y estimación de complejidad (Q1) y novedad de los analistas (Q2).**
...
|:--|:--|:--|:--|:--|
| Q1 | ciuo | menor | (46,  4, 0) &nbsp; = **(92, 8, 0) %** | (49, 1, 0) &nbsp; &nbsp; = **(98, 2, 0) %**  | 
| Q1 | ciuo | mayor | (26, 18, 6) = **(52, 36, 12) %**  | (27, 14, 9) &nbsp; = **(54, 28, 18) %**  | 
| Q1 | caenes | menor | (49, 1, 0) &nbsp; = **(98, 2, 0) %**  | (43, 3, 4) &nbsp; &nbsp; = **(86, 6, 8) %**  | 
| Q1 | caenes | mayor | (38, 5, 7) &nbsp; = **(76, 10, 14) %**  | (20, 8, 17) &nbsp; = **(40, 16, 34) %**  | 
| Q2 | ciuo | menor | (35, 15, 0) = **(70, 30, 0) %** | (49, 1, 0) &nbsp; &nbsp; = **(98, 2, 0) %**  | 
| Q2 | ciuo | mayor | (7, 31, 12) = **(14, 62, 24) %**  | (22, 20, 8) &nbsp; = **(44, 40, 16) %** | 
| Q2 | caenes | menor | (49, 1, 0) &nbsp; = **(98, 2, 0) %**  | (44, 2, 4) &nbsp; &nbsp; = **(88, 4, 8) %**  | 
| Q2 | caenes | mayor | (37, 8, 5) &nbsp; = **(74, 16, 10) %**  | (20, 8, 17) &nbsp; = **(40, 16, 34) %**  | 
...
model = ciuo, revision_score = menor, codificador 1. Acc = 0.98
...
model = ciuo, revision_score = menor, codificador 2. Acc = 1.0
...
model = ciuo, revision_score = menor, codificador 1 vs 2. Acc = 0.98
...
model = ciuo, revision_score = mayor, codificador 1. Acc = 0.30
...
model = ciuo, revision_score = mayor, codificador 2. Acc = 0.28
...
model = ciuo, revision_score = mayor, codificador 1 vs 2. Acc = 0.74
...
model = caenes, revision_score = menor, codificador 1. Acc = 0.92
...
model = caenes, revision_score = menor, codificador 2. Acc = 0.94
...
model = caenes, revision_score = menor, codificador 1 vs 2. Acc = 0.94
...
model = caenes, revision_score = mayor, codificador 1. Acc = 0.12
...
model = caenes, revision_score = mayor, codificador 2. Acc = 0.12
...
model = caenes, revision_score = mayor, codificador 1 vs 2. Acc = 0.70
...
::: {.incremental .medium-par}
- **Correlación entre nuestras métrica y clasificación realizada por los analistas.**
...
|:--|:--|:--|:--|
| ciuo | menor | codificador 1 vs model | 0.98 | 
| ciuo | menor | codificador 2 vs model | 1.0 | 
| ciuo | menor | codificador 1 vs 2 | 0.98 |
| ciuo | mayor | codificador 1 vs model | 0.30 | 
| ciuo | mayor | codificador 2 vs model | 0.28 |
| ciuo | mayor | codificador 1 vs 2 | 0.74 |
| caenes | menor | codificador 1 vs model | 0.92 | 
| caenes | menor | codificador 2 vs model | 0.94 |
| caenes | menor | codificador 1 vs 2 | 0.94 |
| caenes | mayor | codificador 1 vs model | 0.12 |
| caenes | mayor | codificador 2 vs model | 0.12 |
| caenes | mayor | codificador 1 vs 2 | 0.70 |
...
::: {.incremental .medium-par}
- Al menos para la coyuntura 2024-01, validamos satisfactoriamente nuestra métrica de drift:
  - **Menor evidencia de drift en registros se correlaciona con menor complejidad y novedad para analistas. ✅**
  - **Mayor evidencia de drift en registros se correlaciona parcialmente con mayor complejidad y novedad para analistas. ✅** 
  - **Evidencia de drift se relaciona inversamente con la performance de los modelos e incluso con la concordancia entre analistas. ✅**
<br><br>
- Trabajo hasta diciembre 2025:
  - **Comprobar validación para siguientes dos coyunturas (2024-02 y 2024-03).** 
<br><br>
- Trabajo desde enero 2026:
  - **Evaluaremos cómo evoluciona en el tiempo la performance de los modelos en base a la clasificación realizada por analistas.**
    - **A partir de dicha evaluación, podremos definir la frecuencia y la forma en que sería conveniente re-entrenar los modelos.**
:::
